...
import { Printer, Plus, Edit, Package, Hash, CheckCircle, Save, X, Upload, Trash2, Camera } from 'lucide-react';
...
useEffect(() => { loadData(); }, []);
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir este equipamento? Ele não aparecerá mais para novas propostas.')) {
      ...
      setEquipments(storage.getEquipments());
    }
  };
  ...
  const handleSave = () => {
    ...
    storage.saveEquipment(currentEq as Equipment);
    setEquipments(storage.getEquipments());
    setIsEditing(false);
  };
