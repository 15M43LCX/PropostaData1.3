...
import {
  ...
  PlusCircle, BarChart3, ChevronRight, Menu, X, CheckCircle2, AlertCircle,
  Kanban, ChevronLeft, ChevronDown
} from 'lucide-react';
...
const AppLayout: React.FC<{ user: User; onLogout: () => void; notification: any }> = ({ user, onLogout, notification }) => {
  ...
  const [collapsed, setCollapsed] = useState(false);   // desktop retrátil
  ...
  // Fecha drawer ao clicar fora (overlay)
...
const Sidebar: React.FC<SidebarProps> = ({
  user, onLogout, collapsed, mobileOpen, onToggleCollapse, onCloseMobile
}) => {
  const location = useLocation();
  ...
  // Classes de largura: desktop collapsed vs expanded, mobile drawer
  const sidebarClass = [
  ...
  return (
    <aside className={sidebarClass}>
      {/* Logo + botão fechar (mobile) / retrair (desktop) */}
      <div className={`flex items-center h-16 border-b border-slate-800/60 px-4 shrink-0 ${collapsed ? 'justify-center' : 'justify-between'}`}>
        {!collapsed && (
          <div className="flex items-center gap-2 text-white min-w-0">
            <div className="w-8 h-8 bg-gradient-to-tr from-blue-600 to-blue-400 rounded-xl flex items-center justify-center shadow-lg shrink-0">
              <BarChart3 size={16} />
            </div>
        ...
        {collapsed && (
          <div className="w-8 h-8 bg-gradient-to-tr from-blue-600 to-blue-400 rounded-xl flex items-center justify-center shadow-lg">
            <BarChart3 size={16} />
          </div>
        )}

        {/* Botão retrair (desktop) */}
        <button
          onClick={onToggleCollapse}
          className="hidden lg:flex p-1.5 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition shrink-0"
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {collapsed && (
                <div className="absolute left-full ml-3 px-2 py-1 bg-slate-700 text-white text-xs font-semibold rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-xl">
                  {item.label}
                </div>
              )}
            </Link>
      ...
      <div className={`border-t border-slate-800/50 p-3 shrink-0 ${collapsed ? 'flex flex-col items-center gap-2' : ''}`}>
        ...
        <button
          ...
          className={[
            'flex items-center gap-3 rounded-xl hover:bg-red-900/20 hover:text-red-400 transition-all text-slate-400',
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="p-10 text-center bg-gradient-to-br from-blue-700 to-indigo-900 text-white relative overflow-hidden">
          ...
          <p className="text-blue-100 opacity-80 relative z-10">Geração de propostas v1.4.0</p>
        </div>
        <div className="p-10">
          <form onSubmit={(e) => { e.preventDefault(); onLogin(email, pass); }}>
            <div className="space-y-5">
              ...
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-3 transform active:scale-95">
                <LogIn size={20} />
                Entrar no Sistema
              </button>
            </div>
          ...
          <div className="mt-8 pt-8 border-t border-slate-100 text-center">
            <button onClick={onReset} className="text-slate-300 hover:text-red-400 text-[10px] uppercase tracking-widest transition-colors">
              Resetar Sistema (Limpar Cache)
            </button>
            <p className="mt-2 text-slate-400 text-[10px] uppercase tracking-widest">Premium Pro Proposals © 2024</p>
          </div>
        </div>
      </div>
    </div>
